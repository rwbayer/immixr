function immixr3(gutterWidth, opacity, userID, accessToken, biggestWidth) {
      ...
      // get instagram feed
        $.ajax({
          ...
          success: function(instagram_data) {
            ...
            draw3(instagramFeed, gutterWidth, biggestWidth);
            // set the opacity to user-specified amount
            $('img.immixr3').css("opacity", opacity);
...
function draw3(instagramFeed, gutterWidth, biggestWidth) {

/* Set Up Variables for Drawing */

  // get drawable location from parent
  var elementWidth = $("div#immixr3").parent().width();
  var elementHeight = $("div#immixr3").parent().height();
...
/* END OF Setting Up Variables for Drawing */
  ...
  $('div#immixr3').append('<div class="immixr3-row '+ (row) + '"></div>');
  currentRow = 'div.immixr3-row.' + row;
  ...
  while(availableHeight > smallWidth) {
    ...
    if((availableWidth < smallWidth) && (availableHeight > (bigWidth + segmentWidth))) {
      ...
      if(availableHeight > (bigWidth + segmentWidth)) {
        dataDivHeight += (bigWidth + segmentWidth);
    ...
    // if there's not enough room for another big row, add a row of the smaller version of images
      ...
      // add the small image
      $(currentRow).append('<img class="immixr3" style="width: ' + smallWidth + 'px; height: auto; position: absolute; top: ' + top + 'px; left: ' + left + 'px;" src="' + instagramFeed[cursor].images.standard_resolution.url + '">');
      ...
      switch(random) {
        ...
        case 0:
          ...
          if(bigWidth > availableWidth)
            break;
             ...
             $(currentRow).append('<img class="immixr3" style="width: ' + bigWidth + 'px; height: auto; position: absolute; top: ' + top + 'px; left: ' + left + 'px;" src="' + instagramFeed[cursor].images.standard_resolution.url + '">');
        ...
        case 1:
          ...
          // special case where cursor wouldn't be able to be incremented, so set manually back to beginning
            ...
            $(currentRow).append('<img class="immixr3" style="width: ' + smallWidth + 'px; height: auto; position: absolute; top: ' + top + 'px; left: ' + left + 'px;" src="' + instagramFeed[cursor].images.standard_resolution.url + '"><img class="immixr3" style="width: ' + smallWidth + 'px; height: auto; position: absolute; top: ' + (top + smallWidth + segmentWidth) + 'px; left: ' + left + 'px;" src="' + instagramFeed[0].images.standard_resolution.url + '">');
             left += (smallWidth + segmentWidth);
             availableWidth -= (smallWidth + segmentWidth);
             cursor = 1;
             break;
  ...
  $('div#immixr3').css({width: elementWidth, height: dataDivHeight, 'margin-top': -(dataDivHeight/2)});
}
