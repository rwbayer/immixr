function immixr2(gutterWidth, opacity, userID, accessToken, biggestWidth) {
      ...
      // get instagram feed
        $.ajax({
          ...
          success: function(instagram_data) {
            ...
            draw2(instagramFeed, gutterWidth, biggestWidth);
            // set the opacity to user-specified amount
            $('img.immixr2').css("opacity", opacity);
...
function draw2(instagramFeed, gutterWidth, biggestWidth) {

/* Set Up Variables for Drawing */

  // get drawable location from parent
  var elementWidth = $("div#immixr2").parent().width();
  var elementHeight = $("div#immixr2").parent().height();
...
/* END OF Setting Up Variables for Drawing */
  ...
  $('div#immixr2').append('<div class="immixr2-row '+ (row) + '"></div>');
  currentRow = 'div.immixr2-row.' + row;
  ...
  while(availableHeight > smallWidth) {
    ...
    // if there's not enough room for another big row, add a row of the smaller version of images
      ...
      // add the small image
      $(currentRow).append('<img class="immixr2" style="width: ' + smallWidth + 'px; height: auto; position: absolute; top: ' + top + 'px; left: ' + left + 'px;" src="' + instagramFeed[cursor].images.standard_resolution.url + '">');
      ...
      switch(random) {
        ...
        case 0:
          ...
          if(bigWidth > availableWidth)
            break;
             ...
             $(currentRow).append('<img class="immixr2" style="width: ' + bigWidth + 'px; height: auto; position: absolute; top: ' + top + 'px; left: ' + left + 'px;" src="' + instagramFeed[cursor].images.standard_resolution.url + '">');
        ...
        case 1:
          ...
          // special case where cursor wouldn't be able to be incremented, so set manually back to beginning
            ...
            $(currentRow).append('<img class="immixr2" style="width: ' + smallWidth + 'px; height: auto; position: absolute; top: ' + top + 'px; left: ' + left + 'px;" src="' + instagramFeed[cursor].images.standard_resolution.url + '"><img class="immixr2" style="width: ' + smallWidth + 'px; height: auto; position: absolute; top: ' + (top + smallWidth + segmentWidth) + 'px; left: ' + left + 'px;" src="' + instagramFeed[0].images.standard_resolution.url + '">');
             left += (smallWidth + segmentWidth);
             availableWidth -= (smallWidth + segmentWidth);
             cursor = 1;
             break;
  ...
  $('div#immixr2').css({width: elementWidth, height: dataDivHeight, 'margin-top': -(dataDivHeight/2)});
}
